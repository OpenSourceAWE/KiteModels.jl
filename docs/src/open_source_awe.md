...
- Install [Julia 1.13 and VSCode](https://ufechner7.github.io/2024/08/09/installing-julia-with-juliaup.html). VSCode is optional, but it is nice to read the examples and the configuration files. You can also use any other IDE or editor if you prefer something else.
...
Users can compose modular symbolic components (Wing, Pulley, Point, Segment, Tether, Winch) via YAML configurations, supporting multi-wing setups and custom geometries more easily than KiteModels.jl's fixed structs like KPS3 or KPS4. Linearization and code generation for control are native, unlike manual adaptations in KiteModels.jl.
