...
@testset verbose = true "KPS4 tests...." begin
    ...
    @testset "calc_particle_forces!  " begin
        ...
        rho = kps4.set.rho_0
        for i = 1:(kps4.set.segments+KiteModels.KITE_PARTICLES+1)
            kps4.forces[i] .= zeros(3)
        end
        bytes = 0
        for i = 1:length(kps4.springs)
            spring = kps4.springs[i]
            kps4.stiffness_factor = 0.5
            kps4.v_wind_tether .= KVec3(8.0, 0.1, 0.0)
            bytes = @allocated KiteModels.calc_particle_forces!(
                kps4,
                pos1,
                pos2,
                vel1,
        ...
        # Python output
        res=[
            [18550.4729309395152086 18550.6132232745367219 18550.6305627766196267]
    ...
    @testset "init                  " begin
        ...
        pos1 = [
            ...
            [76.5905217485472747 0.0 134.6435550484500823]
            [78.5650000003636109 0.0 136.0785716987731178]
    ...
    function split_res(res, kps4)
        particles = kps4.set.segments+KiteModels.KITE_PARTICLES
    ...
    @testset "initial_residual      " begin
        ...
        X00 = zeros(SimFloat, 2*(kps4.set.segments+KiteModels.KITE_PARTICLES-1)+2)
        y0, yd0 = KiteModels.init(kps4, X00)
        residual!(res, yd0, y0, kps4, 0.0)
        res_pos, res_vel = split_res(res, kps4)
    ...
    @testset "inner_loop!            " begin
        ...
        forces = [
            [-1.0581480474703286 -0.020155169648329054 0.6109409799927648]
    ...
    function init2(kps4)
        ...
        kps4.set.alpha = 1.0/7.0
        length = 150.0
        kps4.segment_length = length/kps4.set.segments
        for i = 1:(kps4.set.segments+KiteModels.KITE_PARTICLES+1)
            kps4.forces[i] .= zeros(3)
        end
        KiteModels.init_springs!(kps4)
    ...
    @testset "test_loop          " begin
        ...
        forces=[
            [-1.058148047467609 -0.020155169648329054 0.6109409799974753]
    ...
    function unpack_add_origin(y::MVector{S,SimFloat}) where {S}
        ...
        pos = SVector{div(T, 6)+1}(if i==1
            SVector(0.0, 0, 0)
        else
            SVector(pos1[:, i-1])
        end for i = 1:(div(T, 6)+1))
        vel = SVector{div(T, 6)+1}(if i==1
            SVector(0.0, 0, 0)
        else
            SVector(vel1[:, i-1])
        end for i = 1:(div(T, 6)+1))
    ...
    function unpack(y::MVector{T,SimFloat}) where {T}
        ...
        pos1, vel1 = part[:, :, 1], part[:, :, 2]
        pos = SVector{div(T, 6)}(SVector(pos1[:, i]) for i = 1:div(T, 6))
        vel = SVector{div(T, 6)}(SVector(vel1[:, i]) for i = 1:div(T, 6))
        pos, vel
    end
    ...
    @testset "test_residual!       " begin
        ...
        kps4.v_wind_gnd .= KVec3(7.0, 0.1, 0.0)
        ...
        y0s = [
            ...
            62.5000000000000142,
            0.000001,
            108.2531754730548244,
            75.0000000000000142,
            0.000001,
            129.9038105676658006,
            76.5905217485472747,
            0.0,
            134.6435550484500823,
            78.5650000003636109,
            0.0,
            136.0785716987731178,
            77.4500000002498865,
            2.4810999999999996,
            134.1473350483994693,
            77.4500000002498865,
            -2.4810999999999996,
            134.1473350483994693,
            0.00000,
    ...
    @testset "test_getters" begin
        ...
        @test all(
            KiteModels.orient_euler(kps4) .≈ [0, 0.5235987756571836, -1.5707963267948966],
        )
    ...
    @testset "test_spring_forces    " begin
        ...
        ref_forces = [
            3.928735076156923e-12,
            3.928735076156923e-12,
            3.928735076156923e-12,
            0.0,
            -3.928735076156923e-12,
            1.1786205228470769e-11,
            2.160277004750972,
            2.1602770047433926,
            2.1602770047441373,
            2.1602770047074573,
